            ...
            <div class="topology-workspace">
              <aside class="topology-resource-tree">
                ...
                <div class="resource-tree-list">
                  <section
                    v-for="platform in topologyPlatforms"
              ...
              <main class="topology-main">
                ...
                <div class="topology-provider-list">
                  <section v-for="platform in topologyPlatforms" :key="platform.id" class="topology-provider-section">
              ...
              <aside class="topology-detail-panel">
                ...
                <template v-else-if="selectedTopologyPlatform">
                  ...
                  <div class="detail-actions">
                    ...
                    <el-button class="reset-btn" @click="loadTopology">
                      刷新
                    </el-button>
                  </div>
                </template>
...
const selectedTopologyPlatform = computed<any | null>(() => {
  ...
  if (!selection) return topologyPlatforms.value[0] || null
  return topologyPlatforms.value.find((item: any) => item.id === selection.platformId) || null
})
...
const ensureTopologySelection = () => {
  ...
  const platform = topologyPlatforms.value[0]
  if (!platform) {
...
const isCurrentTopologySelectionValid = () => {
  ...
  const platform = topologyPlatforms.value.find((item: any) => item.id === selection.platformId)
...
const selectTopologyPlatform = (platform: any) => {
  ...
  if (topologyPlatformId.value !== platform.id) {
    topologyPlatformId.value = platform.id
  }
  trendScopeType.value = 'platform'
  loadPlatformTrend()
}
...
const selectTopologyCluster = (platform: any, cluster: any) => {
  ...
  selectTrendCluster(platform.id, cluster.id)
}
...
const selectTopologyHost = (platform: any, cluster: any, host: any) => {
  ...
  trendClusterId.value = cluster.id
}
...
const openGuestsFromTopology = () => {
  const platform = selectedTopologyPlatform.value
  if (!platform) return
  guestQuery.platformId = platform.id
  guestQuery.clusterId = selectedTopologyCluster.value?.id || undefined
  guestQuery.page = 1
