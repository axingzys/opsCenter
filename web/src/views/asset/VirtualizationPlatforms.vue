  ...
  <div class="virtualization-page">
    ...
    <el-tabs v-model="activeTab" class="main-tabs">
      ...
      <el-tab-pane label="拓扑视图" name="topology">
        <VirtualizationTopologyExplorer
          v-model:platform-id="topologyPlatformId"
          v-model:trend-scope-type="trendScopeType"
          v-model:trend-cluster-id="trendClusterId"
          :platform-options="platformOptions"
          :loading="topologyLoading"
          :platforms="topologyPlatforms"
          :current-trend-platform-id="currentTrendPlatformId"
          :trend="platformTrend"
          :trend-loading="platformTrendLoading"
          :trend-range="platformTrendRange"
          :trend-cluster-options="trendClusterOptions"
          :trend-panel-title="trendPanelTitle"
          :selected-trend-metrics="selectedTrendMetrics"
          :trend-metric-options="trendMetricOptions"
          :sync-jobs="topologySyncJobList"
          :sync-jobs-loading="topologySyncJobLoading"
          @refresh="loadTopology"
          @change-trend-range="handlePlatformTrendRangeChange"
          @change-trend-metrics="handleTrendMetricChange"
          @open-guests="openGuestsFromTopology"
          @load-sync-jobs="loadTopologySyncJobs"
        />
      </el-tab-pane>
...
import VirtualizationTopologyExplorer from './components/VirtualizationTopologyExplorer.vue'
...
const topologySyncJobLoading = ref(false)
const topologySyncJobList = ref<any[]>([])
...
const loadTopologySyncJobs = async (platformId?: number) => {
  const targetPlatformId = platformId || currentTrendPlatformId.value
  if (!targetPlatformId) {
    topologySyncJobList.value = []
    return
  }

  topologySyncJobLoading.value = true
  try {
    const res: any = await listVirtualizationSyncJobs(targetPlatformId, { page: 1, pageSize: 8 })
    topologySyncJobList.value = res.list || []
  } finally {
    topologySyncJobLoading.value = false
  }
}
...
const openGuestsFromTopology = (payload: { platformId: number; clusterId?: number }) => {
  guestQuery.platformId = payload.platformId
  guestQuery.clusterId = payload.clusterId || undefined
  guestQuery.page = 1
