...
func (s *HTTPServer) RegisterRoutes(r *gin.RouterGroup) {
	databases := r.Group("/databases")
	{
		...
		databases.POST("/protection-profiles/:id/validate", s.authMiddleware.RequireMenuPermission(permDatabaseBackupRun), s.service.ValidateProtectionProfile)
		databases.POST("/protection-profiles/:id/run-restore-drill", s.authMiddleware.RequireMenuPermission(permDatabaseRestoreRun), s.service.RunProtectionProfileRestoreDrill)
		databases.POST("/protection-wizards/mysql-pitr/preview", s.authMiddleware.RequireMenuPermission(permDatabaseBackupView), s.service.PreviewMySQLPITRWizard)
		databases.POST("/protection-wizards/mysql-pitr/apply", s.authMiddleware.RequireMenuPermission(permDatabaseBackupCreate), s.service.ApplyMySQLPITRWizard)
		databases.POST("/protection-wizards/postgresql-barman/preview", s.authMiddleware.RequireMenuPermission(permDatabaseBackupView), s.service.PreviewPostgresBarmanPITRWizard)
		databases.POST("/protection-wizards/postgresql-barman/apply", s.authMiddleware.RequireMenuPermission(permDatabaseBackupCreate), s.service.ApplyPostgresBarmanPITRWizard)
		...
		databases.POST("/runner-hosts/:id/agent-config-snippet", s.authMiddleware.RequireMenuPermission(permDatabaseRunnerToolScript), s.service.GenerateRunnerAgentConfigSnippet)
		databases.POST("/runner-hosts/:id/agent-install", s.authMiddleware.RequireMenuPermission(permDatabaseRunnerToolInstall), s.service.InstallRunnerAgent)
		databases.POST("/runner-hosts/:id/agent-upgrade", s.authMiddleware.RequireMenuPermission(permDatabaseRunnerToolInstall), s.service.UpgradeRunnerAgent)
		databases.POST("/runner-hosts/:id/agent-restart", s.authMiddleware.RequireMenuPermission(permDatabaseBackupRun), s.service.RestartRunnerAgent)
		databases.GET("/runner-hosts/:id/agent-logs", s.authMiddleware.RequireMenuPermission(permDatabaseRunnerToolView), s.service.GetRunnerAgentLogs)
