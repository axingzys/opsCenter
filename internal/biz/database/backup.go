...
func (uc *UseCase) toBackupRecordVO(item *DatabaseBackupRecord, taskName, instanceName string) *DatabaseBackupRecordVO {
	...
	return &DatabaseBackupRecordVO{
		...
		CheckpointBackupType:     item.CheckpointBackupType,
		ArtifactState:            item.ArtifactState,
		ArtifactCacheURI:         item.ArtifactCacheURI,
		SyntheticSourceRecordIDs: item.SyntheticSourceRecordIDs,
		SupersededByRecordID:     item.SupersededByRecordID,
		PurgeEligibleAt:          formatTime(item.PurgeEligibleAt),
		ProtectedUntil:           formatTime(item.ProtectedUntil),
		Status:                   item.Status,
